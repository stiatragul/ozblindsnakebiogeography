...
# In this version, we classify the species as either arid or non-arid, no widespread stages. 
# Classification is based on distribution of 'species'. I plotted ALA distribution on biome map of Australia in QGIS. 
# Species that were previously classifed as widespread includes the following:
# Anilios_bituberculatus - distributed in the arid zone but also many many occurence in coastal areas in southern Australia. Classified as non-arid.
# Anilios_diversus - mostly in Top End with some distribution in the interior. Classified as non-arid
# Anilios_grypusET - occur in eastern edge of the arid biome. Also occur in tropical savannah. Classified as non-arid.
...
state <- read.csv("data/intermediate_data/geohisse/arid_nonarid_binary.csv", header = T)
...
prop_df <- read.csv("data/intermediate_data/geohisse/2022_species_list_arid_nonarid_binary.csv", header = T)
...
mod5 <- GeoHiSSE(phy = phy, data = state, f=fraction,
                 turnover=turnover, eps=eps,
                 hidden.states=FALSE, trans.rate=trans.rate.mod,
                 turnover.upper=100, trans.upper=20, # need to change trans.upper to 20 instead of 10 for model to run. 
                 sann=FALSE, 
...
# save.image(file = 'data/intermediate_data/geohisse/geohisse_binary.Rdata')
load('data/intermediate_data/geohisse/geohisse_binary.Rdata')
