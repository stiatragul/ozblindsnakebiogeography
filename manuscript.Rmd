...
The four SSE models vary by whether they allow diversification rates to be associated with geographic states (range-dependent or range-independent) and whether they allow an unmeasured trait ('hidden states') to influence diversification rates [@beaulieuDetecting2016;@caetanoHidden2018]. 
...
Models 3 and 4 are derivations of models 1 and 2 but also allow unmeasured trait ('hidden states') to contribute to diversification rates (see Table GeoHiSSE table). By incoporating hidden states, we are able to assess whether geographic range or an unmeasured trait contributes more to the observed diversification. For each model, we assumed uniform taxon sampling for 89% in arid lineages, 71% in non-arid lineages, and 83% in widespread lineages based on number of species in the phylogeny and all currently described species of _Anilios_. We then calculated AIC to identify the best-fitting models. Following the approach in @caetanoHidden2018, we averaged the estimates of diversification parameters and ancestral geographic range estimate across all models according to their AIC weights. 
...
All BioGeoBEARS analyses inferred tropical grassland as the ancestral biome of *Anilios*. We identified the DEC+_j_+_x_ model as the most preferred biogeographical history model (delta AICc = -4.47, see Table-supp-BioGeoBEARS). Likelihood ratio test (LRT) indicates that both founder events (+*j* parameter) and distance between biome (+x parameter) together provides statistically significant improvement over the simpler DEC model, DEC+x  and DEC+j (X^2^, DF = 2, D-statistic = 26.7, p-value < 0.001). We found that most _Anilios_ species are found entirely or at least partially in the tropical grassland biome (n = 20) followed by the arid biome (n = 13) and mediterranean biome (n = 10). We also observed that closely related species tend to share the same biome (\@ref(fig:fig-biogeobears). 

Based on 50 BioGeoBEARS biogeographic stochastic mapping of our best model, we inferred the tropical grassland biome to be the ancestral biome of Australian _Anilios_ (\@ref(fig:fig-biogeobears)). However, the ancestor leading to crown *Anilios* is ambiguous. 
We estimated an average of 71.2 ± 1.3 (mean ± SD) total anagenetic and cladogenetic events occured throughout the phylogeny. The majority of these events were anagenetic dispersal (31.2 ± 1.3 events) and cladogenetic founder events (6.1 ± 1.5 events) (see supp-summary_counts_bsm_table). 
...
We found that the exponential environment-dependent speciation and constant extinction (BEnvDCST) with aridity index derived from @scoteseAtlas2021 provided the best fit to the phylogeny (AICc = 238.43; see Table-RPANDA). This best-fitting model indicates that increase aridity is exponentially associated with decrease speciation rate (lambda = 1.32, alpha = -0.14) and a constant background extinction (mu = 0.23). The second and third most competitive models (delta AICc < 2) also found increase aridity is exponentially associated with decrease speciation rate but without estimating extinction (Table-RPANDA-TOP10). Among exponential environment-dependent models, we found Australia-specific paleoclimate data provides a better fit to our phylogeny than global paleoclimate (see supp-TABLE_FULL_RPANDA_model_fit). 

## Diversification rates between arid and non-arid biomes

Results from our ClaDS analysis show that branch-specific diversification rates gradually decrease towards the present (fig-ClaDS). Mean branch-specific rates did did not differ between arid, non-arid, and widespread lineages (fig-ClaDS). 

We applied GeoHiSSE models to evaluate whether speciation, extinction, and turnover rates differ between lineages in arid, non-arid, and widespread ranges. We found that the range-independent model fit the data better than models where diversification rates were depdendent on geographic range (see supp-table-geohisse). The best-fitting geographic range independent model included a hidden unobserved state, indicating diversification rates is better explained by a hidden unobserved state. Compared to the three other models, the best-fitting model accounts for 0.95 of the AIC weight and had AIC value 7.02 units lower than that of the second best-fitting model (see supp-table-geohiss). The weighted average parameter estimates from all models indicate both speciation rates and turnover rates did not differ between arid and non-arid ranges (REF FIGURE; see supplementary XX for net diversification rate and extinction rates). 

\newpage

# Discussion

With 47 recognised species, the mostly Australian-endemic genus *Anilios* is currently the most diverse genus of scolecophidia in the world. In this study we inferred the extant species in Australia has a tropical grassland origin. Within a few million years, this ancestral lineage split with one clade staying in the tropical grassland biome while the other transitioned into the arid biome. Subsequent transitions from these two lineages resulted in the distribution of biomes we observed across the phylogeny today. We also found that paleo aridity index provides the best fit to diversification dynamics observed in our blind snake tree. Although aridification has been shown to accelerate diversification rates in other groups of vertebrates (CITE), we found no such difference for Australian *Anilios* blind snakes. 
...
After their arrival to Australia, the ancestral _Anilios_ initially diversified rapidly (<1 million years) in the tropical grassland biome. The first transition out of the tropical grassland biome were to the Tropical Forest biome (for the branch leading to _A. silvia_) and the arid biome (\@ref(fig:fig-biogeobears))a. 

We found that the initial transition into the arid biome around 20 MYA pre-dated the continent wide aridification that intensified up in the late Miocene [@byrneBirth2008]. 

The Pilbara region has both lineages that have deep coalescent histories and shallower divergences (_A. ammodytes_ and _A. ganei_)
(Reference to phylogeography supplementary figure)

Arid environemnts is heterogenous and some areas have acted as refugia during glaciation, which may cause isolation and subsequent speciation. 

The western part of Australia dried up earlier than the east around 20 MYA (CITE Mitzy).   

Which indicates that blind snakes may be pre-adapted to arid environment. This pattern has been observed in XXX [@oliverPhylogenetic2017]. 
...
The rates of diversification slowed down as Australia started to become more arid
